...
class CalendarComponent extends Component {
  constructor(props) {
    ...
    this.handleKeyAltPageDownPressed = this.handleKeyAltPageDownPressed.bind(
      this
    );
    this.keyHandlerMap = {
  ...
  moveFocusedDay(increment) {
    ...
    const { minCalendarDate, maxCalendarDate } = getCalendarBounds(
      minDate,
      maxDate
    );
...
export default function Calendar({ locale, ...rest }) {
  return (
    <Consumer>
      {(values) => (
        <CalendarComponent locale={getLocale(values, locale)} {...rest} />
      )}
    </Consumer>
  );
}
